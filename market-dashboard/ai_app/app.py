...
def data_sources_page(request: Request):
    ...
    for item in sources:
        ...
        if item["source_key"] == "cninfo":
            ...
            extra = f"<label>模式<select name='config_mode'>{mode_options}</select></label><label>官方 API Path<input name='config_api_path' value='{esc(config.get('api_path') or '')}' placeholder='/api/...'></label><label>Access Token 参数名<input name='config_token_param' value='{esc(config.get('token_param') or 'access_token')}'></label><label>Access Key 参数名<input name='config_access_key_param' value='{esc(config.get('access_key_param') or 'accessKey')}'></label><label>Access Secret 参数名<input name='config_access_secret_param' value='{esc(config.get('access_secret_param') or 'accessSecret')}'></label><label>代码参数名<input name='config_symbol_param' value='{esc(config.get('symbol_param') or 'scode')}'></label><label>开始日期参数<input name='config_start_date_param' value='{esc(config.get('start_date_param') or 'sdate')}'></label><label>结束日期参数<input name='config_end_date_param' value='{esc(config.get('end_date_param') or 'edate')}'></label><label>请求方法<select name='config_method'>{method_options}</select></label><label>Access Key（留空不变）<input type='password' name='secret_access_key' autocomplete='off'></label><label>Access Secret（留空不变）<input type='password' name='secret_access_secret' autocomplete='off'></label><label>Access Token（留空不变）<input type='password' name='secret_access_token' autocomplete='off'></label>"
        secret_label = "Token/API Key（留空不变）" if item["source_key"] != "cninfo" else "备用单字段密钥（通常不用填）"
        forms.append(f"<article class='panel'><form class='settings-form' action='/api/settings/data-sources/{esc(item['source_key'])}' method='post'><div class='section-head'><h2>{esc(item['display_name'])}</h2><span class='muted'>{esc(item['source_key'])} · {configured}</span></div><label class='check'><input type='checkbox' name='enabled' value='true' {checked}> 启用</label><label>抓取天数<input type='number' min='1' max='60' name='fetch_days' value='{esc(item.get('fetch_days'))}'></label>{extra}<label>{secret_label}<input type='password' name='secret' autocomplete='off'></label><button type='submit'>保存</button></form></article>")
...
async def api_update_data_source(source_key: str, request: Request):
    ...
    if source_key == "cninfo":
        cninfo_secret = {
            "access_key": str(payload.get("secret_access_key") or "").strip(),
            "access_secret": str(payload.get("secret_access_secret") or "").strip(),
            "access_token": str(payload.get("secret_access_token") or "").strip(),
        }
        cninfo_secret = {key: value for key, value in cninfo_secret.items() if value}
        if cninfo_secret:
            secret = json.dumps(cninfo_secret, ensure_ascii=False, separators=(",", ":"))
