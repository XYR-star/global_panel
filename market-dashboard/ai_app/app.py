...
def data_sources_page(request: Request):
    ...
    for item in sources:
        ...
        if item["source_key"] == "cninfo":
            ...
            extra = f"<label>模式<select name='config_mode'>{mode_options}</select></label><label>官方 API Path<input name='config_api_path' value='{esc(config.get('api_path') or '')}' placeholder='/api/...'></label><label>Access Token 参数名<input name='config_token_param' value='{esc(config.get('token_param') or 'access_token')}'></label><label>Access Key 参数名（通常留空）<input name='config_access_key_param' value='{esc(config.get('access_key_param') or '')}'></label><label>Access Secret 参数名（通常留空）<input name='config_access_secret_param' value='{esc(config.get('access_secret_param') or '')}'></label><label>代码参数名<input name='config_symbol_param' value='{esc(config.get('symbol_param') or 'scode')}'></label><label>开始日期参数<input name='config_start_date_param' value='{esc(config.get('start_date_param') or 'sdate')}'></label><label>结束日期参数<input name='config_end_date_param' value='{esc(config.get('end_date_param') or 'edate')}'></label><label>请求方法<select name='config_method'>{method_options}</select></label><label>Access Key（留空不变）<input type='password' name='secret_access_key' autocomplete='off'></label><label>Access Secret（留空不变）<input type='password' name='secret_access_secret' autocomplete='off'></label><label>Access Token（可留空自动获取）<input type='password' name='secret_access_token' autocomplete='off'></label>"
